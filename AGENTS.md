...
- CI fetches public GitHub repos at build time (`scripts/sync-github-repos.mjs`) and appends unknown non-fork repos. Handwritten catalog wins. Featured and `hidden` flags stay in `site.ts`. Hidden catalog entries are omitted from public pages until they are unhidden. The live site stays static HTML.
